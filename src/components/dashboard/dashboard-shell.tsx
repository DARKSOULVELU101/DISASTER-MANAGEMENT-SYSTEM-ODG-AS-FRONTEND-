...
export function DashboardShell() {
  ...
  const selectClass =
    "h-10 min-w-[132px] cursor-pointer appearance-none rounded-md border border-line bg-ivory-light pl-3.5 pr-8 text-[13px] font-medium text-ink outline-none transition-colors hover:border-brand-400 focus:border-brand-400";
    ...
    <div className="flex flex-wrap items-center gap-2.5">
      {(
        ...
        <label key={key} className="relative">
          ...
          <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-brand-700">
            ▼
          </span>
        </label>
      ))}
      ...
      {VIEWS.map((v, i) => {
        ...
        return (
          <button
            ...
            className={`group relative flex items-center gap-3 rounded-md px-3.5 py-2.5 text-left transition-colors ${
              isActive ? "text-brand-300" : "text-ivory-light/55 hover:text-ivory-light"
            }`}
          >
            {isActive && (
              <motion.span
                layoutId="nav-indicator"
                className="absolute inset-0 rounded-md border border-brand-400/30 bg-brand-400/10"
                transition={{ type: "spring", stiffness: 420, damping: 34 }}
              />
            )}
            <span
              className={`tabular relative z-10 text-[10.5px] font-semibold ${
                isActive ? "text-brand-400" : "text-ivory-light/35"
              }`}
            >
  ...
  const brand = (
    <div className="flex items-center gap-3">
      <div className="flex h-9 w-9 items-center justify-center rounded-md border border-brand-400/40 bg-charcoal-800">
        <GMark size={22} />
      </div>
      <div>
        <div className="font-display text-[16px] font-semibold leading-none tracking-tight text-ivory-light">
          GENVOUCH
        </div>
        <div className="mt-1 text-[9px] font-medium uppercase tracking-[0.24em] text-brand-400">
          Technologies Pvt
        </div>
      </div>
    </div>
  ...
  const datasetCard = (
    <div className="rounded-md border border-brand-400/25 bg-charcoal-800 p-4">
      <div className="flex items-center gap-2">
        <span className="h-1.5 w-1.5 bg-brand-400 live-dot" />
        <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-brand-300">
          The Ledger
        </span>
      </div>
      <p className="mt-2 text-[12px] leading-relaxed text-ivory-light/60">
        India Disaster Management OGD · {data?.totals.india.records ?? "—"} records · 2014–2023
      </p>
      <Link
        href="/"
        className="mt-3 inline-flex items-center gap-1.5 text-[12px] font-semibold text-brand-300 hover:text-brand-200"
      >
        <ArrowLeft size={13} weight="bold" /> Back to the broadsheet
      </Link>
    </div>
  );
  ...
  return (
    <div className="min-h-dvh bg-ivory text-ink">
      ...
      {/* Desktop folio spine — charcoal chrome */}
      <aside className="plate-charcoal fixed inset-y-0 left-0 z-30 hidden w-[260px] flex-col border-r border-charcoal-700 lg:flex">
        <div className="h-1 w-full bg-gradient-to-r from-brand-800 via-brand-400 to-accent" />
        <Link href="/" className="px-6 pb-2 pt-6">
          {brand}
        </Link>
        <div className="mx-6 my-5 h-px bg-gradient-to-r from-brand-400/50 to-transparent" />
        <div className="flex-1 overflow-y-auto px-3 scroll-thin">{navList}</div>
        <div className="mx-5 mb-6">{datasetCard}</div>
      </aside>
      ...
      <AnimatePresence>
        {navOpen && (
          <>
            ...
            />
            <motion.aside
              className="plate-charcoal fixed inset-y-0 left-0 z-50 flex w-[280px] flex-col border-r border-charcoal-700 lg:hidden"
              initial={{ x: -300 }}
              animate={{ x: 0 }}
              exit={{ x: -300 }}
            ...
            >
              <div className="flex items-center justify-between px-5 pb-2 pt-5">
                ...
                <button
                  onClick={() => setNavOpen(false)}
                  className="rounded-md p-2 text-ivory-light/70 hover:bg-charcoal-800"
                  aria-label="Close navigation"
                >
                  <X size={18} />
                </button>
              </div>
              <div className="mx-5 my-4 h-px bg-gradient-to-r from-brand-400/50 to-transparent" />
              <div className="flex-1 overflow-y-auto px-3 scroll-thin">{navList}</div>
              <div className="mx-5 mb-6">
                <p className="text-[12px] leading-relaxed text-ivory-light/60">
                  India Disaster Management OGD · 2014–2023
                </p>
                <Link href="/" className="mt-2 inline-flex items-center gap-1.5 text-[12px] font-semibold text-brand-300">
                  <ArrowLeft size={13} weight="bold" /> Back to the broadsheet
                </Link>
              </div>
            </motion.aside>
      ...
      <div className="flex min-h-dvh flex-col lg:pl-[260px]">
        <header className="glass-nav sticky top-0 z-20 border-b border-line">
          <div className="mx-auto flex max-w-[1500px] flex-wrap items-center gap-3 px-4 py-3.5 sm:px-6">
            <button
              ...
              className="rounded-md border border-line bg-ivory-light p-2 text-ink lg:hidden"
            ...
            <div className="mr-auto min-w-0">
              ...
              <h1 className="truncate font-display text-[20px] font-semibold leading-tight tracking-tight text-ink sm:text-[22px]">
                {activeView.label}
              </h1>
            </div>
            <div className="hidden items-center gap-2 rounded-full border border-brand-400/40 bg-ivory-light px-3 py-1 sm:flex">
              <span className="live-dot relative flex h-2 w-2">
                <span className="relative inline-flex h-2 w-2 rounded-full bg-brand-500" />
              </span>
              <span className="text-[11px] font-semibold text-brand-800">Live OGD Data</span>
            </div>
        ...
        <main className="mx-auto w-full max-w-[1500px] flex-1 px-4 py-6 sm:px-6 sm:py-8">
          {loadError ? (
            <div className="card-elev mx-auto max-w-md p-10 text-center">
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-md border border-brand-400/50 bg-ivory-light">
                <GMark size={30} />
              </div>
              ...
              <button
                ...
                className="mt-5 rounded-md bg-charcoal px-5 py-2.5 text-[13px] font-semibold text-ivory-light transition-colors hover:bg-charcoal-700"
              >
                Reload
              </button>
            </div>
        ...
        <footer className="border-t border-line bg-ivory-light/70">
