    ...
    >
      ...
      {nodes.map((n, i) => (
        <motion.circle
          ...
          cy={n.y}
          r="1.3"
          fill="rgba(45,156,255,0.65)"
          initial={{ scale: 0, opacity: 0 }}
          animate={{ scale: [0, 1, 1.6, 1], opacity: [0, 0.9, 0.35, 0.9] }}
          transition={{ duration: 3.4, delay: 0.3 + i * 0.18, repeat: Infinity, ease: "easeInOut" }}
        />
      ))}
    </svg>
  );
    ...
    <div className="pointer-events-none absolute inset-0 flex items-center justify-center" aria-hidden="true">
      {particles.map((p) => (
        <motion.span
          ...
          transition={{ duration: 1.6, delay: 0.35 + p.id * 0.06, ease: [0.16, 1, 0.3, 1] }}
        />
      ))}
      <motion.span
...
export function GenvouchLoader({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const done = setTimeout(onComplete, START_EXIT + 650);
    return () => {
    ...
    return (
      <div className="fixed inset-0 z-[70] flex items-center justify-center bg-ivory">
        <div className="flex flex-col items-center gap-4">
          <GMark size={72} glow />
          <div className="font-sans text-2xl font-semibold tracking-tight text-brand-800">
            GENVOUCH
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="radial-blue absolute inset-0" />
      <NetworkNodes className="opacity-60" />
          ...
          <AnimatePresence mode="wait">
            ...
            {(step === "word" ||
              ...
              >
                <div className="relative">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <h1
                      ...
                      style={{ fontSize: "clamp(2.1rem, 6vw, 3.1rem)", lineHeight: 0.95, letterSpacing: "-0.045em" }}
                    >
                ...
                <div
                  className="mt-3 font-sans font-medium text-brand-500"
                ...
                >
                  {TECHNOLOGIES.split("").map((ch, i) => (
                    <motion.span
                      ...
                      transition={{ duration: 0.32, delay: i * 0.03, ease: "easeOut" }}
                    >
                      {ch}
                    </motion.span>
                  ))}
      ...
      {/* Step 6 — brand edge light that leads the curtain up */}
      <motion.div
        className="absolute inset-x-0 bottom-0 h-[3px] bg-gradient-to-r from-brand-800 via-brand-500 to-accent"
        initial={{ opacity: 0 }}
